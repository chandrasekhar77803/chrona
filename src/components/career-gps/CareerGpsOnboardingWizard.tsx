...
export interface OnboardingData {
  ...
  githubUrl: string;
  linkedinUrl: string;
  roadmapType?: 'Fast Track' | 'Balanced' | 'Mastery';
}
...
export const CareerGpsOnboardingWizard: React.FC<Props> = ({
  initialCompany = 'Google',
  initialRole = 'AI Engineer',
  onComplete
}) => {
  ...
  // Question 1: Dream Career
  const [careerGoal, setCareerGoal] = useState<string>(initialRole || 'AI Engineer');
  const [customGoal, setCustomGoal] = useState<string>('');

  // Question 2: Dream Company
  const [dreamCompany, setDreamCompany] = useState<string>(initialCompany || 'Google');
  ...
  const handleFinish = () => {
    ...
    setTimeout(() => {
      const finalGoal = careerGoal === 'Other' ? customGoal || 'AI Engineer' : careerGoal;
      const finalCompany = dreamCompany === 'Custom Company' ? customCompany || 'Google' : dreamCompany;
    ...
    <div className="py-8 max-w-2xl mx-auto glass-panel p-6 sm:p-10 rounded-3xl border border-indigo-500/30 bg-slate-950/95 shadow-2xl space-y-8 animate-fadeIn">
      ...
      <div className="flex items-center justify-between pb-6 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-white">AI Career Discovery Wizard</h2>
            <p className="text-xs text-slate-400">Question {step} of 10 • Personalized AI Mentor Setup</p>
          </div>
        </div>
        ...
        <div className="space-y-6 animate-fadeIn">
          <div className="space-y-2">
            <h3 className="text-xl font-black text-white flex items-center gap-2">
              ...
              <span>Question 1: What is your dream career?</span>
            </h3>
            <p className="text-xs text-slate-400">Select the specific role you want to excel in.</p>
          </div>
          ...
          {careerGoal === 'Other' && (
            <input
              type="text"
              placeholder="Enter your custom dream career..."
              value={customGoal}
              onChange={e => setCustomGoal(e.target.value)}
              className="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-indigo-500/40 text-xs text-white"
            />
          )}
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-6 animate-fadeIn">
          ...
          {dreamCompany === 'Custom Company' && (
            <input
              type="text"
              placeholder="Enter your custom dream company..."
              value={customCompany}
              onChange={e => setCustomCompany(e.target.value)}
              className="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-purple-500/40 text-xs text-white"
            />
          )}
        </div>
      )}
      ...
      {step === 3 && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {['3 Months', '6 Months', '9 Months', '12 Months', '18 Months', '24 Months'].map(time => (
              <button
                key={time}
      ...
      {step === 6 && (
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={addCustomSkill} className="px-4 py-2 rounded-xl bg-slate-800 text-xs font-bold text-purple-300 cursor-pointer">
              + Add
            </button>
          </div>
        </div>
      ...
      {step === 7 && (
        <div className="space-y-6 animate-fadeIn">
          ...
          {hasProjects && (
            <textarea
              placeholder="Briefly mention your project details (e.g. Full-Stack E-commerce app, Chatbot with RAG...)"
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="p-8 rounded-3xl border-2 border-dashed border-slate-800 hover:border-indigo-500/50 bg-slate-900/60 text-center space-y-3">
            ...
            <input
              ...
              id="resume-input"
            />
            <label htmlFor="resume-input" className="inline-block px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs cursor-pointer">
              Choose File
            </label>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-slate-800">
        ...
        {step < 10 ? (
          ...
          >
            ...
            <span>🚀 Generate My Personalized Career GPS</span>
          </button>
        )}
      </div>
    </div>
